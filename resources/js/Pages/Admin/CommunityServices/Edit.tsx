...
export default function Edit({ service }: { service: ServiceItem }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.community-services.update', service.id));
    };

    return (
        <AdminLayout title="Edit Pengabdian Masyarakat">
            <Head title="Admin - Edit Pengabdian" />
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl space-y-6 rounded-2xl border border-cream-300/40 bg-surface-0 p-6 shadow-sm md:p-8">
                <div>
                    <h2 className="text-lg font-bold text-ink-900">Edit Program Pengabdian</h2>
                    <p className="mt-0.5 text-xs text-navy-700">Perbarui informasi program pengabdian masyarakat.</p>
                </div>
