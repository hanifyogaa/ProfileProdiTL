...
export default function Create() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.plos.store'));
    };

    return (
        <AdminLayout title="Tambah PLO Baru">
            <Head title="Admin - Tambah PLO" />
            ...
            <form onSubmit={handleSubmit} className="bg-surface-0 border border-cream-300/40 p-6 md:p-8 rounded-2xl shadow-sm space-y-6 max-w-2xl">
                <div>
                    <h2 className="text-lg font-bold text-ink-900 mb-1">Tambah PLO Baru</h2>
                    <p className="text-xs text-navy-700">Program Learning Outcome — kompetensi yang harus dimiliki mahasiswa saat lulus. Ditampilkan di halaman /kurikulum/capaian-pembelajaran.</p>
                </div>
                ...
                <div className="pt-4 border-t border-cream-300/40 flex justify-end space-x-3">
                    ...
                    >
                        ...
                        <span>{processing ? 'Menyimpan...' : 'Simpan PLO'}</span>
                    </button>
                </div>
            </form>
        </AdminLayout>
