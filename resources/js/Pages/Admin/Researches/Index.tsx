...
import { ArrowLeft, Calendar, FlaskConical, Pencil, Plus, Search, Tag, Trash2 } from 'lucide-react';
...
export default function Index({ researches, categories, filters }: IndexProps) {
    const [search, setSearch] = useState(filters.search ?? '');
    const [category, setCategory] = useState(filters.category ?? '');

    const applyFilters = () => {
        router.get(route('admin.researches.index'), { search, category }, { preserveState: true, replace: true });
    };

    const handleDelete = (id: number) => {
        if (!confirm('Hapus data penelitian ini?')) return;
        router.delete(route('admin.researches.destroy', id));
    };

    return (
        <AdminLayout title="Manajemen Penelitian">
            <Head title="Admin - Penelitian" />

            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 className="font-display text-2xl font-bold text-ink-900">Penelitian (Riset)</h2>
                    <p className="mt-0.5 text-sm text-navy-700">{researches.total} data penelitian terdaftar</p>
                </div>
                <Link
                    href={route('admin.researches.create')}
            ...
            <div className="mb-5 flex flex-wrap gap-3">
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input
                        ...
                        placeholder="Cari judul, tim peneliti..."
                ...
                <select
                    value={category}
                    onChange={(e) => { setCategory(e.target.value); router.get(route('admin.researches.index'), { search, category: e.target.value }, { preserveState: true, replace: true }); }}
            ...
            <div className="overflow-hidden rounded-2xl border border-cream-300/40 bg-surface-0 shadow-sm">
                <table className="w-full text-sm">
                    <thead className="bg-surface-50/50 text-left">
                        <tr>
                            <th className="px-5 py-3.5 font-semibold text-navy-700">Judul</th>
                            ...
                            <th className="hidden px-5 py-3.5 font-semibold text-navy-700 lg:table-cell">Tim</th>
                            <th className="px-5 py-3.5 text-right font-semibold text-navy-700">Aksi</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-cream-300/20">
                        {researches.data.length > 0 ? researches.data.map((item) => (
                            <tr key={item.id} className="hover:bg-surface-50/40 transition-colors">
                                <td className="px-5 py-4">
                                    <div className="flex items-center gap-3">
                                        {item.image ? (
                                            ...
                                            <div className="bg-brand-700/6 flex size-10 shrink-0 items-center justify-center rounded-lg">
                                                <FlaskConical className="text-brand-700/30 size-5" />
                                            </div>
                                        )}
                                        <div>
                                ...
                                <td className="hidden px-5 py-4 text-xs text-navy-700/70 lg:table-cell">
                                    <span className="line-clamp-1">{item.team ?? '—'}</span>
                                </td>
                                <td className="px-5 py-4 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <Link
                                            href={route('admin.researches.edit', item.id)}
