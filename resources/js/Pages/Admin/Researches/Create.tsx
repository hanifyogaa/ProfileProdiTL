...
export default function Create() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.researches.store'));
    };

    return (
        <AdminLayout title="Tambah Penelitian">
            <Head title="Admin - Tambah Penelitian" />
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl space-y-6 rounded-2xl border border-cream-300/40 bg-surface-0 p-6 shadow-sm md:p-8">
                <div>
                    <h2 className="text-lg font-bold text-ink-900">Tambah Penelitian</h2>
                    <p className="mt-0.5 text-xs text-navy-700">Catat proyek riset atau publikasi ilmiah dosen/mahasiswa Prodi Teknik Logistik.</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-cream-300 bg-surface-0 px-4 py-2.5 text-sm outline-none focus:border-brand-700 focus:ring-1 focus:ring-brand-700/20"
                        />
                        {errors.year && <p className="text-xs font-medium text-red-600">{errors.year}</p>}
                    </div>
                    <div className="space-y-1">
                ...
                <div className="space-y-1">
                    ...
                    <input
                        ...
                        placeholder="Contoh: Dr. Ahmad Fauzan, M.T. · Prodi Teknik Logistik"
                        className="w-full rounded-xl border border-cream-300 bg-surface-0 px-4 py-2.5 text-sm outline-none focus:border-brand-700 focus:ring-1 focus:ring-brand-700/20"
                    />
                </div>
                ...
                <ImageUpload
                    label="Gambar / Foto Penelitian (Opsional)"
                ...
                <div className="flex justify-end gap-3 border-t border-cream-300/40 pt-4">
                    ...
                    >
                        ...
                        {processing ? 'Menyimpan...' : 'Simpan Penelitian'}
                    </button>
                </div>
            </form>
        </AdminLayout>
